...
int asMain()
{
	int i,j,x,y,userInput,controlVar = 1,step = 0,emptyCounter,openedCell = 0;
	...
	do
	{
		...
		if (emptyCounter == 0) controlVar = -5; /* Finish game if there isn't any closedEmpty cell left */
		else
		{
			...
			/* ---------------------------------------- */
			if (controlVar > 0) openedCell += controlVar; /* opened cell counter */
			step++ ; /* Move counter */
		}
	}	while (controlVar >= 0);
...
int openCell(cell grid[][GRIDSIZE], int x, int y)
{
	int i,j,k = 0; /* k = -2 if location is illegal */
	if (isLocationLegal(x, y))
	{
		...
		case closedEmpty :
			...
			for (i = -1; i < 2; ++i) /* For loop for 3 rows */
				for (j = -1; j < 2; ++j) /* For loop for 3 columns */
					if ((grid[x+i][y+j] == closedEmpty) && (y+j < GRIDSIZE) &&  /* If conditions makes sure */
						(x+i < GRIDSIZE) && (x+i >= 0) && (y+j >= 0)) /* cell is empty and in boundaries */
							k += openCell(grid,x+i,y+j); /* Turns cell to empty if it's viable */
			break;
		...
		case closedMined :
			...
			k = -1; /* Return value in case of hitting the bomb */
			break;
